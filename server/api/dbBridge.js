...
function init_db() {
    ...
    sql =
        "CREATE TABLE messages (messageID INT AUTO_INCREMENT PRIMARY KEY,sender VARCHAR(255) , receiver VARCHAR(255) ,FOREIGN KEY (sender) REFERENCES users(phone),FOREIGN KEY (receiver) REFERENCES users(phone), content VARCHAR (255),  time TIME)";
    con.query(sql, function (err, result) {
...
module.exports = {init_db, getContactsByUserId, addUser, getUsers, getUser, usernamePasswordMatch};
